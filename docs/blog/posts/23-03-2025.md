...
An exploitable SSRF vulnerability that allows connections to external third-party systems could be used to execute forward attacks. These attacks may appear to originate from the organization hosting the vulnerable application. For example, an attacker could use a server vulnerable to SSRF to scan ports on another company's internal network or even launch a DoS attack against an external service, making it seem as though the attack is coming from the organization hosting the vulnerable application.

Furthermore, SSRF attacks are not limited to the HTTP protocol. In certain cases, an attacker might attempt to use other URI schemes, such as `file://`, to access local files on the server. If the server does not properly validate the provided URLs, this could allow access to sensitive files on Linux systems. Although the `file://` scheme is not always enabled, it remains an attack vector that should be considered when implementing security measures.
...
As we've seen, the server receives a URL provided by a user, in this case `https://example.com/image.jpg`, then makes an HTTP request to that URL to verify if it contains an image. Finally, if the provided URL accesses a resource, in this case with the header **Content-Type: image/\***, it will be considered valid.

The problem lies in the fact that the **server blindly trusts** the URLs provided by users, allowing arbitrary *HTTP* requests to be made.

If an attacker supplies a URL controlled by them or that points to an internal resource of the server, such as `http://internal-server/sensitive-data`, the attacker will not directly see the server's response, but may be able to deduce if the request was successful by observing certain behaviors:
...
In addition, if the requested resource does not exist, the server's response will be faster compared to an existing resource.

To verify if the request made from the server to the attacker's target is correct, **out-of-band (OOB)** techniques are employed, such as monitoring DNS or HTTP requests to domains controlled by the attacker.  For example: `http://internal-server.sensitive-data.attacker-domain.com`. As we can see in this malicious URL, `attacker-domain` is a domain controlled by the attacker; therefore, the vulnerable server will attempt to resolve this malicious domain and make an HTTP request to it. Because of this, the attacker can monitor their own DNS or web server to confirm that the request was made. An attacker can use a domain they control (for example, `attacker.com`) and configure DNS records to record all DNS queries made to that domain. Then, inject a URL like `http://unique-id.attacker.com`. If the server is vulnerable, it will attempt to resolve this domain (generating a DNS request) and subsequently make an HTTP request. The attacker will see an entry in their logs, confirming the vulnerability.

Furthermore, services like [Canarytokens](https://canarytokens.org/generate){:target="_blank"} can be used to generate unique tokens that alert when they are accessed.
