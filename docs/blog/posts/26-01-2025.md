---
title: "DNS Overview and Setup"
summary: “This article provides a comprehensive guide on understanding and configuring a local DNS server using BIND9. It starts by explaining the fundamentals of DNS, including its hierarchical structure and how it translates domain names into IP addresses. The article introduces the concept of a local DNS server, detailing its benefits such as faster query responses through caching, filtering unwanted content, and improving network security. It then walks readers through practical steps to set up BIND9, implement DNSSEC for secure queries, and block specific domains (e.g., Instagram) by creating custom zone files. The guide also explores extending the setup to a router or a Raspberry Pi, enabling network-wide control over DNS queries for enhanced security, productivity, or parental control. Perfect for users looking to take charge of their network's DNS behavior.”
date: 2025-01-26
tag:
  - DNS
  - Setup
...
In this article, we will explore what a **DNS** is and how this hierarchy works so that you can better understand its crucial role in internet browsing. From the root servers that act as the starting point, through **TLD** and authoritative servers, we come to the local **DNS**, the key piece that connects this whole global system to your home or business network. Get ready to understand how you can set up your own **_local DNS_** and optimise your environment's browsing!
...
When you type a web address (domain name) into your browser, your computer doesn’t understand that name directly. Instead, it needs to find the corresponding IP address, which is the "real" address of the server hosting the website. The DNS performs this lookup process, known as **name resolution**.

## **The Hierarchy of DNS Servers**

<figure markdown="span">
    ![Hierarchy of DNS Servers](../../assets/Hierarchical-DNS-system.png){ width="550" }
...
- When you type a URL in your browser, your device sends a request to a **recursive resolver**. This is usually provided by your Internet Service Provider (ISP) or a third-party service like Google Public DNS or Cloudflare.
...
In this blog, we'll use the **BIND9 DNS server** to set up a DNS server directly on our computer. This hands-on approach will let us experience what it’s like to have a fully functional DNS server running within our own network—right from the comfort of our own machine!
...
To block specific domains using BIND9, you can create a **custom zone file** that redirects queries for those domains to an invalid or non-existent address, effectively blocking access. Here's a step-by-step guide:
...
3. Add the following content to the zone file:
...
<figure markdown="span">
    ![Blocked Instagram](../../assets/noInstagram.png){ width="600" }
