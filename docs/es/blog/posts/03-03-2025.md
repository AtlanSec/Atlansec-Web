---
title: "Introducciónn a las Plantillas de Nuclei"
summary: "Explicación básica de las plantillas de Nuclei con un ejemplo"
date: 2025-03-03
tag:
    - Ciberseguridad
    - Pentesting
...
En este articulo, Exploraremos como funcionan las plantillas de [**Nuclei**](https://docs.projectdiscovery.io/templates/introduction) , detallando el **proceso de creación y configuración de vulnerabilidades personalizadas**. Aprenderas los fundamentos, desde la configuración de la información básica y la elaboración de peticiones HTTP precisas hasta el uso de **variables**, **payloads**, **matchers** y **extractors** para pruebas dinámicas. Además, un ejemplo práctico demuestra cómo simular un servidor vulnerable y detectar una vulnerabilidad utilizando una plantilla personalizada **Nuclei**.

<!-- more -->

## ¿Que es una plantilla de Nuclei?

Una plantilla de Nuclei es un archivo con formato YAML que, rellenando secciones específicas, permite estandarizar y automatizar la detección de vulnerabilidades sin tener que reescribir código cada vez. En este artículo, trabajaremos con el protocolo más común para plantillas: HTTP.

- **Información básica:** Identificador, nombre, autor, gravedad y descripción.
- **Requests:** Configuración de las solicitudes HTTP que se enviarán al objetivo.
...
Esta sección categoriza la plantilla utilizando campos de metadatos:
...
- **referencia:** URL a documentación de apoyo, CVE o recursos externos.
...
*[Payloads](https://docs.projectdiscovery.io/templates/protocols/http/http-payloads)* permiten realizar pruebas de inyección o métodos de fuerza bruta definiendo listas de valores, como cadenas, caracteres especiales o patrones. Cada carga útil se itera y se inyecta en los parámetros de solicitud designados.
...
- **json:** Aplica sintaxis tipo JQ para extraer datos de respuestas JSON.
...
## Guía práctica: Servidor Web Vulnerable y Plantilla de Nucli


### Iniciar el docker vulnerable

Vamos a utilizar [este](https://github.com/appsecco/vulnerable-apps/tree/master/dsvw) servidor con un par de vulnerabilidades para probar y practicar nuestros nuevos conocimientos adquiridos sobre **plantillas Nuclei**.
...
En este primer ejemplo hay una **boolean SQLI** que puede ser según la tabla se llena o no, usando el *matcher* **dsl** para mirar la longitud del cuerpo de la respuesta podemos saber si existe o no **boolean SQLI**. 

---

### Crear una plantilla de Nuclei para detectar la boolean SQLI

Para averiguar la longitud de la contraseña, utilizaremos un payload (de 1 a 9) codificada en la plantilla, aunque también se podría utilizar un archivo con mas *payloads* para hacerlo más dinámico.
...
Nuclei enviará una petición al servidor y al encontrar la longitud correcta, mostrará un resultado indicando que la prueba se ha realizado correctamente y la carga útil que corresponde a la longitud de la contraseña.


## Conclusión

Nuclei es una potente herramienta para automatizar la detección de vulnerabilidades, y mediante la creación de tus propias plantillas, puedes optimizar significativamente tu análisis. 

Después de este primer ejemplo práctico, desde Atlansec te animamos a que sigas explorando el resto de vulnerabilidades del sitio para conocer mejor la herramienta. 

Cuanto más practiques, más eficaces serán tus auditorias de seguridad.
