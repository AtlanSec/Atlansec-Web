...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['comentario'])) {
    $comentario = trim($_POST['comentario']); // Eliminamos espacios en blanco al inicio y al final
...
!!! danger
    No hacemos uso de `mysqli_real_escape_string` debido a que NO protege contra SQL Injection por sí solo, ya que depende de la configuración de `set_charset()`. Si consultamos la [documentación oficial](https://www.php.net/manual/es/mysqli.real-escape-string.php){:target="_blank"} veremos que nos advierte que si la codificación no se establece correctamente, el escape puede no funcionar como se espera, permitiendo ataques de inyección en algunas configuraciones.
...
4) Dale una revision al concepto de DOM XSS (https://portswigger.net/web-security/cross-site-scripting/dom-based) , lo dejo sin corregir pa que investigues un poco mas y cuando lo revise con los cambios que haga ya si le meto mano yo
5) Como bonus (da pa otro/otros articulos) echale un ojo a por que he puesto que en moviles y aplicaciones de escritorio un XSS es una ruina -->
