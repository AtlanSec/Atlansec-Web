...
- **Ejecución de código malicioso**: Meidante técnicas avanzadas dependientes de la situiación un atacante podría llegar a ejecutar código arbitrario en el contexto del servidor.
...
Ahora, consideremos una aplicación web más robusta, donde los payloads basicos para **LFI** no funciona, ya que la web contiene este código:
...
La transición de una vulnerabilidad de **Local File Inclusion (LFI)** a **Remote Code Execution (RCE)** es una técnica avanzada que los atacantes pueden emplear para comprometer un sistema por completo. Estos metodos son bastante dependeintes del lenguaje en el que se escribe la aplicación y de configruaciónes especificas del servidor, los métodos más comunes para lograr pasar de un LFI a RCE son los siguientes:
...
De igual manera y dependiendo del log que podamos leer se puede inyectar el código a ejecutar en los logs del servicio web (a traves del User-Agent por ejemplo), en los logs del SSH (mediante el nombre de usuario) e incluso en logs custom de la aplicación mediante diversos parametros.
