...
La transición de una vulnerabilidad de **Local File Inclusion (LFI)** a **Remote Code Execution (RCE)** es una técnica avanzada que los atacantes pueden emplear para comprometer un sistema por completo. Este tipo de ataque se basa en aprovechar la capacidad de incluir archivos locales para ejecutar código malicioso en el servidor.

#### Acceso a los archivos de log del servidor

Un paso inicial común es verificar si los archivos de log del servidor son accesibles mediante LFI. Por ejemplo, los logs de Apache suelen estar ubicados en `/var/log/apache2/access.log`. Si el atacante puede incluir este archivo, podría inyectar código malicioso en los logs y ejecutarlo.

Ejemplo de URL para intentar acceder al archivo de log:
```url
http://dominio.com/pruebas/index.php?file=../../../../../../var/log/apache2/access.log
```

Si el archivo es accesible, el atacante puede proceder a inyectar código PHP en los logs, utilizando métodos como modificar el `User-Agent` en una solicitud HTTP. Esto convierte el archivo de log en un vector para ejecutar comandos arbitrarios en el servidor.
...
#### Escalando de LFI a RCE

1. **Inyección en los logs del servidor**:
    - El atacante envía una solicitud HTTP con un `User-Agent` modificado que contiene código PHP malicioso, como:
      ```bash
      curl -A "<?php system('id'); ?>" http://dominio.com/index.php
      ```
    - Esto genera una entrada en el archivo de log que incluye el código PHP.

2. **Incluir el archivo de log mediante LFI**:
    - Usando la vulnerabilidad LFI, el atacante incluye el archivo de log en la aplicación:
      ```url
      http://dominio.com/pruebas/index.php?file=../../../../../../var/log/apache2/access.log
      ```
    - Si el servidor procesa el archivo de log como código PHP, ejecutará el código malicioso inyectado.
